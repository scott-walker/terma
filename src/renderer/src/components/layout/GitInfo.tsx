...
export const GitInfo = memo(function GitInfo({ cwd, isSshMode }: GitInfoProps): JSX.Element | null {
  ...
  return (
    <div className="flex min-w-0 cursor-default items-center gap-2 text-sm text-fg-muted" draggable={false}>
      <GitBranch size={15} strokeWidth={1.8} className="shrink-0 text-fg-muted" />
      {gitInfo.url ? (
        <a
          href={gitInfo.url}
          draggable={false}
