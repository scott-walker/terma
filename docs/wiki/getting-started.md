...
2. **TabBar** — панель табов с одним начальным табом
3. **Терминал** — полноэкранный xterm.js, подключённый к вашему системному shell
4. **StatusBar** — статусная строка с информацией о текущем каталоге

Начинайте вводить команды — терминал полностью функционален.

При повторном запуске сессия восстановится автоматически (табы, панели, рабочие каталоги).

### Попробуйте

- `Ctrl+Shift+T` — создать новый таб
- `Ctrl+Shift+D` — разделить панель вертикально
- `Ctrl+Shift+E` — разделить панель горизонтально
- `Ctrl+Shift+B` — открыть файловый менеджер в сплите
- `Ctrl+Shift+A` — переключить панель в режим агента
